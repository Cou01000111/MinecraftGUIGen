...
module.exports = { setOptionData, setOptionData };

function setOptionData(path, resourcePackPath) {
  if (fs.existsSync(path)) {
    $("#gameOptionInput").val(path);
    setOptionDataMain(resourcePackPath);
  } else {
    ew.gameDirNotFound();
  }
}
...
function setOptionDataMain(resourcePackPath) {
...
function toStringFromKeyConfig(options, version) {
  ...
  if (version == "1.13") {
    var keycode = gjd.getKeyCode1_13();
    options.forEach((option) => {
      console.log(option);
      stringArr.push(keycode[option]);
    });
  } else if (version == "1.12.2") {
    var keycode = gjd.getKeyCode1_12_2();
    options.forEach((option) => {
      console.log(option);
      stringArr.push(keycode[option]);
    });
